...
type UnbrandField<T> = T extends string & { [Key in BrandKey]: any }
	? string
	: T extends number & { [Key in BrandKey]: any }
		? number
		: T extends symbol & { [Key in BrandKey]: any }
			? symbol
			: T;
...
export function assertTableSchema<TTable extends Table>(table: TTable) {
	return <TSchema extends z.ZodType>(
		_schema: MatchingSchema<TTable, TSchema>,
	) => table;
}
